...
// Realizando a busca aux
int buscaAux(PONT atual, char* palavra, int n, int pos)
{
	if (pos == n) //Se está na última letra da palavra retorna o contador
	{
	...
	int auxi;
	auxi = palavra[pos] - 'a';
	if (atual->filhos && atual->filhos[auxi]) //Confere se os filhos existem
		{
			return buscaAux(atual->filhos[auxi], palavra, n, pos+1); //Chama a função novamente para próxima palavra.
		}
	}
 return 0; // retorna 0 se os if's falharem
}

int buscarPalavra(PONT raiz, char* palavra, int n){
	return buscaAux(raiz, palavra, n, 0);
}
...
*/
void InserirAux(PONT atual, char* palavra, int n, int pos)
{
	if (pos == n)
	{
		atual->contador += 1;
	...
	{
		int i;
		int flag = 0; // flag que criaNo caso o arranjo de filhos já exista
		int auxi = palavra[pos] - 'a'; // auxi seria a posição do arranjo correspondente a letra desse loop
		if (atual->filhos)// caso em que ja existe o arranjo
		{
			for (i=0;i<LETRAS;i++)
			{
				if (atual->filhos[i] && i == auxi)
				{
					flag = 1;
					InserirAux(atual->filhos[i], palavra, n, pos+1);
				}
			}

		if (flag == 0)
		{
			atual->filhos[auxi] = criarNo();
			InserirAux(atual->filhos[auxi], palavra, n, pos+1);
		}
		}
		else //não existe o arranjo de filhos
		{
...
*/
int excluirTodasAux(PONT raiz, char* palavra, int n, int pos)
{
	if (pos == n)
	{
		if (raiz->filhos == NULL && raiz->contador >0) //exclui o nó se não tiver filhos e o contador for maior que 0
		{
			free(raiz);
			return true;
		}
		else // não faz nada se o contador for igual a 0
		{
			raiz->contador = 0;
			return false;
		}
	...
	if (pos < n)
	{
		bool apagar = false; //variavel que indica se houve exclusão, começa sendo false
		int i;
		int auxi;
		int cont= 0; //conta a quantidade de filhos em um arranjo

		auxi = palavra[pos] -'a';
			if(raiz->filhos && raiz->filhos[auxi] != NULL)
			{

				if (raiz->filhos[auxi])
				{
				   apagar = excluirTodasAux(raiz->filhos[auxi], palavra, n, pos +1); //Confere se houve a exclusão da próxima letra da palavra
				}

				if (apagar == true) //Significa q a palavra posterior foi apagada
				{
					raiz->filhos[auxi] = NULL;
					for (i=0;i<LETRAS;i++)
					{
						if (raiz->filhos[i] != NULL) cont ++; //Verificando se o arranjo só possui valores nulls
					}
					if (cont == 0) //Se não tiver filhos o if é executado
					{
						free(raiz->filhos); //apaga o arranjo
						raiz->filhos = NULL; // aponta para NULL agora
						if (raiz->contador == 0 && pos !=0) //confere se o nó não faz parte de outra palavra
						{
							free(raiz);
							return true;
						}
			...
			else // retorna false se a raiz n tiver filhos
			{
				return false;
			}
...
int excluirAux(PONT raiz, char* palavra, int n, int pos)
{
	if (pos == n)
	{
		if (raiz->filhos == NULL && raiz->contador == 1)
		{
			raiz->contador = 0; //linha provavelmente desnecessária
			free(raiz);
			return true;
		}
	...
	if (pos < n)
	{
		...
		auxi = palavra[pos] -'a';
			if(raiz->filhos)
			{
				for (i=0;i<LETRAS;i++)
				{
					if (raiz->filhos[i] && i == auxi)
					{
						apagar = excluirAux(raiz->filhos[auxi], palavra, n, pos +1);
					}
				}
				if (apagar == true)
				{
